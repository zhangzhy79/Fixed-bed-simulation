...
select=[1 2 3];                     % 参与吸附的组分
% 以及载气氦气（He）的吸附参数设置
% 气体基本参数设置
M = [44 16 28];                     % CO2，CH4，N2的摩尔质量，单位：克每摩尔（g/mol）
M_C = 2;                            % 载气He的摩尔质量，单位：克每摩尔（g/mol）
% 气体流量参数设置
v_L = [117 110 121];                % CO2，CH4，N2的流量，单位：毫升每分钟（mL/min）
v = v_L*1e-6/60/pi/Rinter^2;        % CO2，CH4，N2的流速，单位：米每秒（m/s）
v_L_C = 351;                        % He的流量，单位：毫升每分钟（mL/min）
% 气体分压参数设置
p_initial = [0.83 0.83 0.83];       % CO2，CH4，N2的初始分压，单位：巴（bar）
p_C_initial = 0.83;                 % 载气He的初始分压，单位：巴（bar）
% 等体积流量下的压强（总压强：5bar）
...
DL = [1.50 1.72 1.70]*1e-5;                 % 轴向质量分散系数，单位：平方米每秒（m^2/s）
